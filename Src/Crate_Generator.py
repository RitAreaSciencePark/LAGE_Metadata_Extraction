...
def generate_folder_rocrate(input_folder):
    ...
    VALID_EXTENSIONS = ('.csv', '.txt', '.json', '.md', '.pod5', '.fastq.gz', '.bam', '.bam.bai', '.xlsx', '.pdf', '.jpeg', '.png', '.zip', '.log', '.html', '.tiff', '.xml', '.xad') # Add more as needed
        ...
        for filename in files:
            ...
            if ext in VALID_EXTENSIONS:
                ...
                if (Extractor_IlluminaSampleSheet.is_illumina_samplesheet(full_path) or 
                    ...
                    detected_labels.add("<b>Sequencing phase</b>: runs performed using Illumina NovaSeq6000")
                    global_sequencer_type = "NOVASEQ" # Anchor found
                elif Extractor_Nanopore.is_nanopore_file(full_path):
                    detected_labels.add("<b>Sequencing phase</b>: runs performed using Oxford Nanopore PromethION")
                    global_sequencer_type = "NANOPORE" # Anchor found
                elif Extractor_BeadStudio.is_beadstudio_file(full_path):
                    detected_labels.add("<b>Sequencing phase</b>: runs performed using Illumina iScan")
                    global_sequencer_type = "ISCAN" # Anchor found  
                elif Extractor_NanoDrop_QC.is_nanodrop_export(full_path):
                    detected_labels.add("<b>Pre-sequencing Quality control phase</b>: NanoDrop UV absorbance spectrum for each sample")
                elif Extractor_SampleReport.is_samples_report(full_path):
                    detected_labels.add("<b>Pre-sequencing Quality control phase</b>: report of technical observations and anomalies detected (both before and after sequencing)")        
                elif Extractor_SampleSheet_xlsx.is_lab_samplesheet(full_path):
                    detected_labels.add("<b>Sample quality assessment phase</b> (Sample and plate metadata): description of samples and plate layout used for sequencing.")    
                elif Extractor_FastQC.is_fastqc_zip(full_path):
                     detected_labels.add("<b>Post-sequencing Quality check phase</b>: FastQC quality control report archive generated by FastQC software Babraham Bioinformatics")        
    ...
    for ext, count in extension_counts.items():
        file_summary_parts.append(f"{count} <b>{ext}</b> files")
    ...
    # --- Build the Dynamic File Summary HTML List ---

    if file_summary_parts:
        file_summary_html = "<ul>" + "".join([f"<li>{part}</li>" for part in file_summary_parts]) + "</ul>"
    else:
        file_summary_html = "<i>No valid data files detected.</i>"

    # --- Build the Instrument/Phase HTML List ---
    if detected_labels:
        # We turn the set into a bulleted list as well
        types_html = "<ul>" + "".join([f"<li>{label}</li>" for label in detected_labels]) + "</ul>"
    else:
    ...
    crate.root_dataset["name"] = f"LAGE Experimental Dataset for the Repository: {input_folder_name}"
    ...
    # Define the quality control activity associated with the TapeStation device    
    run_tapestation = crate.add(ContextEntity(crate, "#tapestation-quality-control-activity", properties={
        "@type": "CreateAction",
        "name": "TapeStation Quality Control",
        "description": "Quality control process for samples before sequencing, which includes electrophoretic analysis using the Agilent TapeStation 4200 to assess DNA/RNA integrity and size distribution.",
    ...
    html_format = crate.add(ContextEntity(crate, "#html-format", properties={
        "@type": "File Format",
        "name": "HTML Document",
    ...
    tiff_format = crate.add(ContextEntity(crate, "#tiff-format", properties={
        "@type": "File Format",
        "name": "TIFF Image",
        "description": "Tagged Image File Format (TIFF) is a flexible raster image format that supports lossless compression.",
        "url": "https://en.wikipedia.org/wiki/TIFF"
    }))
    
    xml_format = crate.add(ContextEntity(crate, "#xml-format", properties={
        "@type": "File Format",
        "name": "XML Document",
    ...
    xad_format = crate.add(ContextEntity(crate, "#xad-format", properties={
        "@type": "File Format",
        "name": "XAD Archive",
        "description": "eXtensible Archive Data (XAD) is a custom archive format used for storing complex datasets with embedded metadata.",
        "url": "https://example.com/xad-format-specification"
    }))

    log_format = crate.add(ContextEntity(crate, "#log-format", properties={
        "@type": "File Format",
        "name": "Log File",
        "description": "Plain text file that records events, processes, or messages generated by software applications or systems.",
    ...
    FORMAT_ENTITY_MAP = {
        ...
        'text/html': html_format.id,
        'image/tiff': tiff_format.id,
        'text/xml': xml_format.id,
        'application/xad': xad_format.id,
        'text/log': log_format.id
   ...
   # ---  Create the Folder (Dataset) Entity ---
    ...
    folder_entity = crate.add(ContextEntity(crate, folder_id, properties={
        ...
        "description":(
            f"<p>Main data directory containing outputs from the following phases:</p>"
            f"{types_html}"
            f"<p>This dataset includes:</p>"
            f"{file_summary_html}"  # The automated bullet list generated from the file summary string
            ),
        ...
        for filename in files:
            ...
            if ext == ".fastq.gz":
                if global_sequencer_type == "NOVASEQ":
                    ...
                    custom_description = "<i>Gzipped FASTQ files generated by the Illumina NovaSeq6000 platform.</i>"
                elif global_sequencer_type == "NANOPORE":
                    assigned_run = run_nano
                    custom_description = "<i>Gzipped FASTQ files generated by Oxford Nanopore PromethION basecalling.</i>"
                else:
                    custom_description = "<i>Gzipped FASTQ sequence data (Sequencer not identified).</i>"
            ...
            # Check for BeadStudio (iScan)
                ...
                custom_description = "<i>Illumina iScan microarray raw data or intensity file.</i>"
            ...
            # Check for Illumina Sample Sheets (NovaSeq)
                  ...
                  Extractor_SampleReport.is_samples_report(full_path)):
                assigned_run = run_novaseq
                custom_description = "<i>Instrument report or sample manifest associated with the Illumina NovaSeq6000 run.</i>"
            ...
            # Check for NanoDrop QC exports
                ...
                custom_description = "<i>Quantitative sample quality control data exported from NanoDrop.</i>"

            # check for SampleSheet.xlsx (Acceptance phase)
            elif Extractor_SampleSheet_xlsx.is_lab_samplesheet(full_path):
                assigned_run = acceptance_activity    
                custom_description = "<i>Excel file containing sample and plate scheme description provided by the researcher during the acceptance phase before sequencing.</i>"
            
            # ... inside the for filename in files loop ...
            elif Extractor_FastQC.is_fastqc_zip(full_path):
                assigned_run = post_qc_activity
                custom_description = "<i>FastQC quality control report archive generated from raw FASTQ sequencing data</i>"
               
            # Check for PDF files (general description, not instrument-specific)
            elif ext == '.pdf':
                assigned_run = qc_activity  
                custom_description = "<i>Technical report or spectral curves obtained during the quality control phase.</i>"
            # Check for JPEG files (general description, not instrument-specific)
            elif ext == '.jpeg':
                assigned_run = qc_activity
                custom_description = "<i>Visual representation (photo) related to sample quality control or lab observation.</i>"            
            elif   ext == '.xad':
                assigned_run = run_tapestation
                custom_description = "<i>Raw analytical data, including electrophoretic signals, run parameters, ScreenTape type, and sample-associated information (sample identifier, position within the run, date and time of analysis).</i>"
            ...
            if not custom_description:
                ...
                if ext == '.csv':
                    custom_description = "<i>Tabular data file containing quantitative results and analysis outputs.</i>"
                elif ext in ['.txt', '.md']:
